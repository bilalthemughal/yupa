...
<div class="panel panel-flat">
    <div class="panel-heading">
        <h5 class="panel-title">Amenity Update
        <a href="{{ route('admin.amenity') }}" class="btn btn-info"><i class="icon-eye8 mr-2"></i>View</a>
        </h5>
    </div>
    <div class="panel-body">
        <form role="form" action="{{ route('admin.amenity.update') }}" method="post" enctype="multipart/form-data" id="addForm">
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        <label>Amenity Name:</label>
                        <input type="hidden" name="id" value="{{ $amenity->id }}">
                        <input type="text" name="name" id="name" class="form-control" placeholder="Amenity Name" value="{{ $amenity->name }}">
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        <label>Icon:</label>
                        <input type="text" name="icon" id="icon" class="form-control" placeholder="Icon" value="{{ $amenity->icon }}">
                        <span class="text-muted">For Icon: <span class="btn btn-xs btn-link" data-toggle="modal" data-target="#modal_remote">Click Here</span></span>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <select name="status" id="status" class="form-control select">
                            ...
                            <option value="0" {{ $amenity->status != 1 ? 'selected' : ''}}>InActive</option>
                        </select>
                    </div>
                </div>
            </div>
            <div class="text-right">
                <button type="submit" class="btn btn-primary"  id="submit">Update Amenity<i class="icon-arrow-right14 position-right"></i></button>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header bg-light border-grey-300">
                        <h5 class="modal-title">Icon List</h5>
