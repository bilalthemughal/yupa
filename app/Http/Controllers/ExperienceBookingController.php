...
//
//		return redirect()->route('kit_withpack', ['secret' => $secret]);
//
	...
	public function travelkit_book(Request $request) {
		...
		if (!isset($kit)) {
			...
			for ($i = 0; $i < count($kit); $i++) {
				...
				$bookingKit->save();
				$subtotal = round($request->price[$i] / config('settings.c_rate', config('system.defualt_curency_rate')), 2);
				$total += $subtotal * $request->quantity[$i];
				$var = 'item' . '_' . $i;
				$kit_item = TravelKit::where('id', $kit[$i])->first();
