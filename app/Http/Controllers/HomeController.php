...
//		$data['destination'] = Destination::latest()->take(4)->get();
        $destination = Destination::all()->random(4);
	...
	public function destination_show($id) {
		...
		$related = $destination->packages;

//		$related = Destination::where('slug', $destination->slug);
//        $related = $related->where('id', '!=', $destination->id)->get();
//		return $related->count();

		if($related->count() > 4){
            $related = $related->get()->random(4);
        }

		return view('fontend.destination_single', compact('destination', 'latest', 'related'));
	}
