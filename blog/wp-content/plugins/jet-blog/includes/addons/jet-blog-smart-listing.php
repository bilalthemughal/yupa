...
class Jet_Blog_Smart_Listing extends Jet_Blog_Base {
	...
	protected function _register_controls() {
		...
		$css_scheme = apply_filters(
			...
			array(
				...
				'post_image'            => '.jet-smart-listing__post-thumbnail',
		...
		$this->add_control(
			'featured_position',
			array(
				'label'   => esc_html__( 'Featured Post Postition', 'jet-blog' ),
				'label_block' => true,
				'type'    => Controls_Manager::SELECT,
				'default' => 'left',
				'options' => array(
		...
		$this->add_responsive_control(
			...
			array(
				'label'      => esc_html__( 'Gloabal Wrapper Margin', 'jet-blog' ),
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .jet-smart-listing__more-wrap' => 'justify-content: {{VALUE}};',
				),
				'separator' => 'before',
			)
		);
	...
	public function __get_arrows() {
		...
		printf( '<div class="jet-smart-listing__arrows">%s</div>', $arrows );
	...
	public function __get_posts() {
		...
		$this->query_data['current_page'] = $query_args['paged'];
	...
	public function __featured_image( $context = 'simple' ) {
		...
		$img    = get_the_post_thumbnail( get_the_ID(), $size, array( 'class' => $class, 'alt' => get_the_title() ) );
		$format = apply_filters(
	...
	public function __post_title( $context = 'simple' ) {
		...
		switch ( $context ) {
			case 'featured':
				...
				if ( 'boxed' === $layout ) {
					$title = sprintf( '%1$s', get_the_title(), get_permalink() );
				} else {
					$title = sprintf( '<a href="%2$s">%1$s</a>', get_the_title(), get_permalink() );
				}
