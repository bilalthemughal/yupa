...
class Jet_Blog_Text_Ticker extends Jet_Blog_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} ' . $css_scheme['widget_title'] . ':after' => 'position:absolute;content:"";width: 0; height: 0; border-style: solid; border-width: {{title_pointer_height.SIZE}}{{title_pointer_height.UNIT}} 0 {{title_pointer_height.SIZE}}{{title_pointer_height.UNIT}} {{SIZE}}{{UNIT}}; border-color: transparent transparent transparent {{title_pointer_color.VALUE}};left: 100%;top:50%;margin-top:-{{title_pointer_height.SIZE}}{{title_pointer_height.UNIT}};z-index: 999;',
				),
			)
		);
	...
	public function __get_current_date( $settings ) {
		...
		printf( $result_format, $icon_html, date( $format ), $hide_classes );
	}
	...
	public function __post_thumbnail( $settings ) {
		...
		the_post_thumbnail( array( $size, $size ), array( 'class' => $class ) );
