...
if ( ! class_exists( 'Galwalking_About_Widget' ) ) {

	/**
	 * Class Galwalking_About_Widget.
	 */
	class Galwalking_About_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display an information about your site.', 'galwalking' );
			$this->widget_id          = 'galwalking_widget_about';
			$this->widget_name        = esc_html__( 'About Galwalking', 'galwalking' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => '',
				...
				'media_id' => array(
					...
					'value'              => '',
					'label'              => esc_html__( 'Logo:', 'galwalking' ),
				),
				'enable_social' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'enable_social' => esc_html__( 'Enable Social Buttons', 'galwalking' ),
					),
				),
				'enable_tagline' => array(
					...
					'options' => array(
						'enable_tagline' => esc_html__( 'Enable Tagline:', 'galwalking' ),
					),
				),
				'content'  => array(
					'type'              => 'textarea',
					'placeholder'       => esc_html__( 'Text or HTML', 'galwalking' ),
					'value'             => '',
					'label'             => esc_html__( 'Content:', 'galwalking' ),
		...
		public function get_social_nav( $wrapper ) {
			...
			if ( is_array( $social_enabled ) && 'true' === $social_enabled['enable_social'] ) {
				$content = sprintf( $wrapper, galwalking_get_social_list( 'widget' ) );
			}
		...
		public function widget( $args, $instance ) {
			...
			$template = galwalking_get_locate_template( 'inc/widgets/about/views/about.php' );
			...
			$src       = wp_get_attachment_image_src( $media_id, apply_filters( 'galwalking_about_widget_image_size', 'medium' ) );
			...
			if ( $tagline_enabled ) {
				$format   = apply_filters( 'galwalking_about_widget_tagline_format', '<p>%s</p>', $this->settings, $this->args );
...
add_action( 'widgets_init', 'galwalking_register_about_widget' );
/**
 * Register about widget.
 */
function galwalking_register_about_widget() {
	register_widget( 'Galwalking_About_Widget' );
}
