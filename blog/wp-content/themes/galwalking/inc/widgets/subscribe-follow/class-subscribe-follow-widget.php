...
class Galwalking_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'galwalking' );
		$this->widget_id          = 'galwalking_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'galwalking' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'galwalking' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'galwalking' ),
				'label' => esc_html__( 'Subscribe Title', 'galwalking' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'galwalking' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'galwalking' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'galwalking' ),
			),
			'subscribe_submit_type' => array(
				...
				'options' => array(
					'text' => array(
						'label' => esc_html__( 'Text', 'galwalking' ),
						'slave' => 'subscribe_submit_type_text',
					),
					'icon' => array(
						'label' => esc_html__( 'Icon', 'galwalking' ),
						'slave' => 'subscribe_submit_type_icon',
					),
				),
				'label'   => esc_html__( 'Subscribe Submit Type', 'galwalking' ),
			),
			'subscribe_submit' => array(
				'type'   => 'text',
				'value'  => esc_html__( 'Subscribe', 'galwalking' ),
				'label'  => esc_html__( 'Subscribe submit label', 'galwalking' ),
			...
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'galwalking' ),
				'label' => esc_html__( 'Subscribe success', 'galwalking' ),
			),
			'subscribe_style' => array(
				...
				'options' => array(
					'subscribe_style' => array(
						'label' => esc_html__( 'Use Custom Subscribe Form Style', 'galwalking' ),
			...
			'subscribe_input_bg' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Subscribe Input Background Color', 'galwalking' ),
				'value'  => '',
				'master' => 'subscribe_style',
			),
			'subscribe_input_bg_alpha' => array(
				'type'       => 'slider',
				'label'      => esc_html__( 'Subscribe Input Background Color Alpha', 'galwalking' ),
				'value'      => 100,
				'max_value'  => 100,
				'min_value'  => 0,
				'step_value' => 1,
			...
			'subscribe_input_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Subscribe Input Text Color', 'galwalking' ),
				'value'  => '',
				'master' => 'subscribe_style',
			),
			'subscribe_btn_style' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Subscribe Button Style', 'galwalking' ),
				'value'   => 'primary',
				'options' => galwalking_get_btn_style_presets(),
				'master'  => 'subscribe_style',
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'galwalking' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'galwalking' ),
				'label' => esc_html__( 'Follow Title', 'galwalking' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'galwalking' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'galwalking' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'galwalking' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'galwalking' ),
				'multi_upload'       => false,
				'master'             => 'background_image',
			),
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'galwalking' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'galwalking' ),
				'value'   => 'center',
				'options' => array(
			...
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'galwalking' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'galwalking' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'galwalking' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'galwalking' ),
				'value'   => 'inherit',
				'options' => array(
		...
		add_action( 'wp_ajax_galwalking_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_galwalking_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'galwalking' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'galwalking' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'galwalking' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'galwalking' ),
		);

		$this->nonce = wp_create_nonce( 'galwalking-subscribe-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		$follow_template            = galwalking_get_locate_template( 'inc/widgets/subscribe-follow/view/follow-view.php' );
		$subscribe_template         = galwalking_get_locate_template( 'inc/widgets/subscribe-follow/view/subcribe-view.php' );
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'galwalking' );
		}
		...
		if ( $this->is_css_required() ) {
			$dynamic_css = galwalking_theme()->dynamic_css;
	...
	public function custom_subscribe_input_style() {
		...
		if ( $input_bg ) {
			...
			$dynamic_utilities = galwalking_theme()->dynamic_css->get_css_functions();
		...
		galwalking_theme()->dynamic_css->add_style(
	...
	public function get_social_nav() {
		return galwalking_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = galwalking_render_icons( $subscribe_submit );

		if ( 'icon' === $this->instance['subscribe_submit_type'] ) {
			$subscribe_submit_icon_format = apply_filters( 'galwalking_subscribe_submit_icon_format', '<i class="fa %s"></i>' );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'galwalking-subscribe-nonce' ) ) {
...
add_action( 'widgets_init', 'galwalking_register_subscribe_follow_widgets' );
/**
 * Register subscribe-follow widget.
 */
function galwalking_register_subscribe_follow_widgets() {
	register_widget( 'Galwalking_Subscribe_Follow_Widget' );
}
